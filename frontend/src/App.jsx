...
import {
  ...
  Clock, Brain, ListChecks, CheckCircle2, XCircle, BarChart3, BookMarked,
} from "lucide-react";
...
export default function App(){
  ...
  useEffect(()=>{ (async()=>{ const s=await store.get(KEY); setState(s||blankState()); })(); },[]);
  ...
  const ctx={ state, update, go, back, goTab, recordSession, recordAttempt };
  let screen;
  switch(route.name){
...
function Progress({ ctx }){
  const { state, update, go }=ctx; const prog=subjectProgress(state);
...
function Admin({ ctx }){
  ...
  return (
    <div>
      ...
      {view==="dash" && <AdminDash state={state} />}
